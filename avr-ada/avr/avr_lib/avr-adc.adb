...
package body AVR.ADC is
   ...
   ADC_Left_Adjust_Result : Boolean renames
     MCU.ADMUX_Bits (MCU.ADLAR_Bit);
   ADC_Enable : Boolean renames
     MCU.ADCSRA_Bits (MCU.ADEN_Bit);
   ADC_Auto_Trigger_Enable : Boolean renames
     MCU.ADCSRA_Bits (MCU.ADATE_Bit);
   ADC_Interrupt_Enable : Boolean renames
     MCU.ADCSRA_Bits (MCU.ADIE_Bit);
   ADC_Start_Conversion : Boolean renames
     MCU.ADCSRA_Bits (MCU.ADSC_Bit);
   ...
   is
      Channel_Mask : constant Nat8 := 2#1110_0000#;
   begin
      MCU.ADMUX := (MCU.ADMUX and Channel_Mask) or Unsigned_8 (Ch);
