...
package body SHT.Comm is
   ...
   begin
      ...
      while B > 0 loop
         Clock_Line_High;
         Wait_2us;
         if Read_Data_Line = AVR.High then
            Value := Value or B;
         end if;
         Clock_Line_Low;
         Wait_2us;
   ...
   begin
      ...
      Clock_Line_Low;
      Wait_2us;
      for I in Unsigned_8 range 1 .. 9 loop
         Clock_Line_High;
         Wait_2us;
         Clock_Line_Low;
         Wait_2us;
   ...
   begin
      ...
      Write_Byte (Command_Code (Status), Err2);
   ...
   begin
      ...
      for I in Unsigned_16 loop
         exit when Read_Data_Line = Low;
      end loop;
      --  or timeout
      if Read_Data_Line = High then E := Timeout_Error; return; end if;

      Raw_Value := ADC_Type (Read_Byte (Send_ACK => True)) * 256;
      Raw_Value := Raw_Value + ADC_Type (Read_Byte (Send_ACK => True));
      Checksum  := Read_Byte (Send_ACK => False);
   end Collect;
